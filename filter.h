...
void solve(Kernel &&kernel, VectorType &&result, VectorType &&source,
           size_t max_iter = 10, size_t restart = 10,
           linear_solver solver = CG) {
  typedef typename std::remove_reference<Kernel>::type::Scalar Scalar;
  switch (solver) {
  case CG: {
    Eigen::ConjugateGradient<typename std::remove_reference<Kernel>::type,
    ...
    std::cout << "CG:    #iterations: " << cg.iterations()
              << ", estimated error: " << cg.error() << " actual error: "
              << (kernel * result - source).norm() / source.norm() << std::endl;
    break;
  }
  case BiCGSTAB: {
    Eigen::BiCGSTAB<typename std::remove_reference<Kernel>::type,
                    Eigen::IdentityPreconditioner>
        bicg;
    ...
    std::cout << "BiCGSTAB:    #iterations: " << bicg.iterations()
              << ", estimated error: " << bicg.error() << " actual error: "
              << (kernel * result - source).norm() / source.norm() << std::endl;
    break;
  }
  case GMRES: {
...
*/
    Eigen::GMRES<typename std::remove_reference<Kernel>::type,
                 Eigen::IdentityPreconditioner>
        gmres;
    ...
    std::cout << "GMRES:    #iterations: " << gmres.iterations()
              << ", estimated error: " << gmres.error() << " actual error: "
              << (kernel * result - source).norm() / source.norm() << std::endl;
    break;
  }
  case DGMRES: {
    Eigen::DGMRES<typename std::remove_reference<Kernel>::type,
                  Eigen::IdentityPreconditioner>
        dgmres;
    ...
    std::cout << "DGMRES:    #iterations: " << dgmres.iterations()
              << ", estimated error: " << dgmres.error() << " actual error: "
              << (kernel * result - source).norm() / source.norm() << std::endl;
    break;
  }
  }
...
typedef Particles<std::tuple<boundary, alpha1, alpha2, boundary, target, inlet,
                             outlet, interior, velocity, knotpot, gradknotpot>,
                  2, std::vector, Kdtree>
    KnotsType;
typedef Particles<std::tuple<boundary, dvelocity, dpressure, velocity, pressure,
                             kernel_constant>,
                  2>
    ComsolType;
typedef Particles<std::tuple<velocity, stokes_velocity, electro_velocity, angle,
                             count, charge>,
                  2>
    ParticlesType;
typedef Particles<
    std::tuple<point_a, point_b, traction, boundary, normal, gradP, charge>, 2,
    std::vector, Kdtree>
    ElementsType;
...
auto gen_spring(I &i, J &j, const double &k, const double &s) {
  ...
  return deep_copy(if_else(dot(dx, dx) == 0, vector(0, 0),
