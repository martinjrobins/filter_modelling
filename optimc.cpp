...
int main(int argc, char **argv) {
  ...
  desc.add_options()("help", "produce help message")(
      ...
      "periodic", po::value<bool>(&periodic)->default_value(true),
      "periodic in x")("D", po::value<double>(&D)->default_value(0.01),
                       "diffusion constant")(
      "particle_rate", po::value<double>(&particle_rate)->default_value(1000.0),
      "particle rate")("react_rate",
                       po::value<double>(&react_rate)->default_value(0.5),
                       "particle reaction rate")(
      "n_min", po::value<unsigned int>(&nmin)->default_value(2), "min n")(
      "n_max", po::value<unsigned int>(&nmax)->default_value(3),
      "max n")("nx", po::value<unsigned int>(&nx)->default_value(10),
               "nx")("fibre_resolution",
                     po::value<double>(&fibre_resolution)->default_value(0.75),
                     "knot resolution around fibre")(
      "seed", po::value<int>(&seed)->default_value(10), "seed")(
      "fibre_number", po::value<int>(&fibre_number)->default_value(5),
      "number of fibres")("fibre_radius",
                          po::value<double>(&fibre_radius)->default_value(0.3),
                          "radius of fibres")(
      "dt", po::value<double>(&dt_aim)->default_value(0.001), "timestep")
      //("filename",
      // po::value<std::string>(&filename)->default_value("optim.out"),
      //"filename")
      ;
  ...
  file << std::setw(15) << "n" << std::setw(15) << "rms_error" << std::setw(15)
       << "max_error" << std::setw(15) << "solve_error" << std::endl;

  const double fibres_charge = 1.0;
  ...
  for (unsigned int n = nmin; n < nmax; ++n) {
    ...
    // fibres
    {
      ...
      for (int ii = 0; ii < fibre_number; ++ii) {
        for (int jj = 0; jj < fibre_number; ++jj) {
          ...
          get<charge>(p) = fibres_charge * normal(generator);
          fibres.push_back(p);
        }
      }
      fibres.init_neighbour_search(domain_min - ns_buffer,
                                   domain_max + ns_buffer,
    ...
    // SETUP KNOTS
    //
    setup_knots(knots, fibres, fibre_radius * 1.01, fibre_resolution, nx,
                domain_min, domain_max, k, false, 8 * 8);
    ...
    auto Akernel = make_greens_kernel_2d1p(alpha, nlambda, nmu, h, domain_min,
                                           domain_max, false);
    auto A = create_dense_operator(comsol, elements, Akernel);

    std::cout << "calculating solution at comsol points...." << std::endl;
    A.get_first_kernel().evaluate(get<velocity>(comsol),
    ...
    double rms_error_v = std::sqrt(std::accumulate(
        std::begin(comsol), std::end(comsol), 0.0,
        [](double accum, const_comsol_reference p) {
          return accum + (get<velocity>(p) - get<dvelocity>(p)).squaredNorm();
        }));

    const double max_error_v = std::accumulate(
        std::begin(comsol), std::end(comsol), 0.0,
        [](double accum, const_comsol_reference p) {
          return std::max(accum,
                          std::max((get<velocity>(p) - get<dvelocity>(p))[0],
                                   (get<velocity>(p) - get<dvelocity>(p))[1]));
        });

    file << std::setw(15) << n << std::setw(15) << rms_error_v << std::setw(15)
         << max_error_v << std::setw(15) << relative_error << std::endl;

    std::cout << "rms errors = " << rms_error_v << ' ' << std::endl;

    std::cout << "max errors = " << max_error_v << ' ' << std::endl;
