...
*/
  ...
  for (int ii = 0; ii < N; ++ii) {
    get<gradP>(elements)[ii] = get<charge>(elements)[ii];
  }

  vtkWriteGrid("BEMelements", 0, elements.get_grid(true));

  std::cout << "assemble knot matrix..." << std::endl;
  auto AknotsSLP = create_dense_operator(
      knots, elements, make_laplace_SLP_2d1p(min, max, false));
  auto AknotsDLP = create_dense_operator(
      knots, elements, make_laplace_gradSLP_2d1p(min, max, false));

  AknotsSLP.get_first_kernel().evaluate(get<knotpot>(knots),
                                        get<gradP>(elements));
  AknotsDLP.get_first_kernel().evaluate(get<gradknotpot>(knots),
                                        get<gradP>(elements));
  ...
  std::transform(std::begin(get<knotpot>(knots)), std::end(get<knotpot>(knots)),
