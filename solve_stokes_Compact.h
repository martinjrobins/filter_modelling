...
#include "filter.h"
    ...
    double kernel(const vdouble2& dx, const double invh) {
    ...
    double kernel_yy(const vdouble2& dx, const double invh) {
        const double r = dx.norm()*invh;
        if (r > 1) return 0.0;
        return 26.0*std::pow(invh,2)*std::pow(r-1.0,8)*((-1.0 + r)*(5.0+3.0*r*(15.0+r*(53.0+77.0*r))) 
                                    + 132.0*(1.0+r*(8.0+21.0*r))*dx[1]*dx[1]*invh*invh); 
    }
    double kernel_xy(const vdouble2& dx, const double invh) {
        const double r = dx.norm()*invh;
        if (r > 1) return 0.0;
        return 3432.0*dx[0]*dx[1]*std::pow(invh,4)*std::pow(r-1.0,8)*(1.0 + r*(8.0 + 21.0*r)); 
    }

    double laplace_xx(const vdouble2& dx, const double invh) {
        const double r = dx.norm()*invh;
        if (r > 1) return 0.0;
        const double r2 = r*r;
        const double r3 = r2*r;
    ...
    double laplace_yy(const vdouble2& dx, const double invh) {
        const double r = dx.norm()*invh;
        if (r > 1) return 0.0;
        const double r2 = r*r;
        const double r3 = r2*r;
    ...
    double laplace_xy(const vdouble2& dx, const double invh) {
        const double r = dx.norm()*invh;
        if (r > 1) return 0.0;
        return 205920.0*dx[0]*dx[1]*std::pow(invh,6)*std::pow(r-1.0,6)*(-3.0+r*(-18.0+49.0*r));
    }
    double laplace2_xx(const vdouble2& dx, const double invh) {
        const double r = dx.norm()*invh;
        if (r > 1) return 0.0;
        const double r2 = r*r;
        const double r3 = r2*r;
    ...
    double laplace2_yy(const vdouble2& dx, const double invh) {
        const double r = dx.norm()*invh;
        if (r > 1) return 0.0;
        const double r2 = r*r;
        const double r3 = r2*r;
    ...
    double laplace2_xy(const vdouble2& dx, const double invh) {
        const double r = dx.norm()*invh;
        if (r > 1) return 0.0;
